...
正如本文第三章（表 3-3）的实验数据显示，即便经过多模态几何约束优化，一个典型的机房孪生场景模型依然包含约 5.12 百万个高斯基元，
其峰值显存占用高达 8.2 GB，模型文件体积动辄数百兆字节（MB）。
...
直接加载该模型会瞬间打满端侧设备的可用显存，引发程序崩溃（Out of Memory, OOM）或产生不可接受的渲染掉帧与卡顿。
