...
近年来，尽管学术界陆续涌现出了一些针对 3DGS 的轻量化压缩方案，例如通过网格引导进行下采样的 Compact 3DGS \cite{lee2024compact3dgs}，以及引入剪枝与知识蒸馏的 LightGaussian \cite{fan2024lightgaussian}。
...
现有典型 3DGS 轻量化方法通常遵循“重要性评估驱动的高斯剪枝--外观属性降阶蒸馏--关键参数低比特量化压缩”的三级流水线。
其核心思想在于：首先依据高斯基元对整体渲染质量的贡献度剔除冗余点，从几何源头缩减模型规模；
随后对高阶球谐函数系数进行紧凑化蒸馏，以降低视角相关外观建模带来的高维参数负担；
最后再对位置、透明度、旋转、尺度及外观等属性进行向量量化或混合精度编码，以进一步压缩模型体积并降低端侧带宽压力。
然而，这类通用压缩流程大多面向标准场景构建，在面对机房环境中普遍存在的高反光、弱纹理、密集遮挡与精细结构边缘时，
仍容易因过强剪枝或低比特离散化而引发局部细节缺失、反光失真与几何边界伪影等问题。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/chap4/fig4_opacity_based_prune.pdf}
    \caption{基于不透明度的直接高斯剪枝局限性示意图}
    \label{fig:opacity_pruning_limitation}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth,height=0.72\textheight,keepaspectratio]{figures/chap4/fig_4_2_gaussian_pruning.pdf}
...
\begin{enumerate}
    ...
    \item 仅开放高维度的球谐函数系数和基础不透明度 $\alpha$ 的梯度更新通道。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth,height=0.65\textheight,keepaspectratio]{figures/chap4/fig_4_5_vq_sh_mapping.pdf}
...
\begin{enumerate}
    \item \textbf{全局码本头：} 优先存储第 4.3 节生成的球谐函数码本 $\mathcal{C}_{sh}$、缩放因子的逆量化对数基底以及场景包围盒（AABB）信息。这部分数据量极小（通常在几百 KB 以内），浏览器能够在极短的时间内优先完成下载与解析，并预先在 GPU 显存中分配相应的 Storage Buffer。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth,height=0.72\textheight,keepaspectratio]{figures/chap4/fig_4_7_webgpu_parallel_decoding.pdf}
    \caption{硬件级并行解码架构}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth,height=0.72\textheight,keepaspectratio]{figures/chap4/fig_4_8_tbdr_optimization.pdf}
    \caption{基于瓦片的渲染优化}
...
本文还选取了目前学术界最新的两项 3DGS 压缩前沿工作作为对比对象：基于网格下采样的 \textbf{Compact 3DGS} \cite{lee2024compact3dgs} 以及基于知识蒸馏与剪枝的 \textbf{LightGaussian} \cite{fan2024lightgaussian}。
...
\label{ch4_subsec:ablation_study}

为了深入验证本章提出的各项轻量化策略的独立贡献，我们在自采集的 \textit{Smart-Room} 场景上进行了严谨的消融实验（Ablation Study）。
以原始 3DGS 为基线，逐步叠加冗余剔除（Pruning）、属性量化（Quantization）等模块，记录核心指标的变化，结果如表 \ref{ch4_tab:ablation_study} 所示。

% 插入表 4-3：消融实验数据对比表
\begin{table}[htbp]
    \centering
    \caption{机房场景下各轻量化模块的消融实验分析}
    \label{ch4_tab:ablation_study}
    \begin{tabular}{lcccc}
        \toprule
...
根据性能柱状图所示，未经压缩的原始 3DGS 模型在轻薄本的核显上勉强维持在 15-20 FPS（重度卡顿），而在平板与智能手机的浏览器中加载即引发显存溢出（OOM）崩溃。
相比之下，加载本文轻量化模型后：
\begin{itemize}
...
在此核心点云的基础上，进一步针对占据极大内存开销的高维球谐函数系数，设计了基于矢量量化（VQ）的特征压缩与码本映射机制，并辅以量化感知微调（QAT）与直通估计器（STE）的训练策略。
