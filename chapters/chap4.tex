...
此时，三维场景表达的“效”——即模型占用的物理存储体积、运行时峰值显存占用（VRAM）以及端侧设备的光栅化渲染帧率（FPS），直接决定了该孪生系统能否真正投入生产环境。

三维高斯溅射（3D Gaussian Splatting, 3DGS）\cite{kerbl20233d}技术虽然凭借其显式的点云表示方法和极其高效的基于瓦片（Tile-based）的并行光栅化渲染管线，
在渲染速度上取得了实质性的突破，突破了隐式神经辐射场（NeRF）的渲染速度瓶颈。
但其极度冗余的高维数据结构与极高的显存消耗，也成为了制约其在端侧大规模部署的致命痛点。
具体而言，原始的 3DGS 算法在拟合大规模且具有复杂遮挡关系的室内场景时，
其自适应密度控制（Adaptive Density Control）机制通常会克隆和分裂出数以百万甚至千万计的高斯椭球体。
在数据结构上，每一个高斯球不仅需要存储基础的三维空间坐标（Position）和不透明度（Opacity），
还必须携带用于表达各向异性几何形状的三维协方差矩阵（Covariance Matrix）参数，
以及用于拟合极度耗费存储空间的、与视角相关的（View-dependent）外观属性的高阶球谐函数（Spherical Harmonics, SH）系数。
这种繁杂的数据量导致一个典型的中等规模机房孪生模型，其文件体积动辄高达数 GB。
...
如图 \ref{fig:edge_deployment_bottleneck} 所示，
...
第 4.2 节将深入论述面向高斯球高维参数（特别是 SH 系数与协方差矩阵）的矢量量化（Vector Quantization）压缩算法及误差补偿机制；
...
如图 \ref{fig:pruning_workflow} 所示，执行阈值过滤后，
...
如图 \ref{fig:memory_footprint} 所示，在单个高斯球总计 236 Bytes 的内存开销中，高维的 SH 系数占据了高达 81.3\% 的比重。
...
如图 \ref{fig:webgpu_decoding_pipeline} 所示，这一架构避免了 CPU 计算浮点数导致的性能枯竭，
...
\textbf{实验硬件与软件环境：} 本文算法的离线训练与压缩阶段（包括冗余剔除、码本聚类与量化感知微调）均部署于搭载 Intel Core i9-13900K CPU、64GB 内存以及单张 NVIDIA GeForce RTX 4090 (24GB VRAM) 的高性能图形工作站上。
软件环境基于 Ubuntu 22.04 操作系统，采用 PyTorch 2.0 深度学习框架与 CUDA 11.8 驱动。而端侧渲染测试则分别在三类典型终端上进行：1) 办公用轻薄笔记本（集成显卡）；2) Apple iPad Pro 平板电脑（测试移动端高分辨率渲染）；3) 搭载最新骁龙旗舰处理器的智能手机。
所有端侧测试均在支持 WebGPU 的标准 Chrome 浏览器下运行。

\textbf{数据集选取：} 由于工业级重型车间数据往往涉及企业机密，本章构建了具有同等空间复杂度与材质特性的“智慧机房与实验室（Smart-Room）”自采集数据集。
该数据集使用高分辨率微单相机与全景设备，围绕机房内的服务器机柜、监控显示器屏幕、玻璃门等区域进行了高重叠率的环绕拍摄与平移扫描，共包含 3 个子场景，平均每个场景包含 300 至 500 张 4K 分辨率图像。
机房场景中大量的显示器反光与金属外壳，能极大程度地检验轻量化算法对复杂光照的保持能力。此外，为了验证算法的普适性，实验还引入了 Mip-NeRF 360 \cite{barron2022mipnerf360} 公共数据集中的室内场景（如 \textit{Room} 和 \textit{Counter}）作为基准参考。
...
在对比基线（Baselines）方面，除了与\textbf{原始 3DGS} \cite{kerbl20233d} 进行直接对比外，
...
\textbf{定性结果分析：} 图 \ref{fig:qualitative_comparison} 选取了机房场景中极具挑战性的“显示器屏幕反光”与“金属机柜交界处”进行局部放大对比。
...
数据证明，基于多维度重要性评估的剔除策略精准识别了隐藏在机柜内部与墙体边缘的“幽灵点”，直接削减了 54.1\% 的几何冗余，但仅损失了 0.07 dB 的 PSNR。
而在点云提纯的基础上，矢量量化（VQ）将庞大的 SH 特征矩阵降维为离散字典索引，完成了绝大部分的体积压缩（从 470MB 压至 48.2MB）。
两者相辅相成，缺一不可。
